...
import { getProfileBySlug, type PublicService } from "@/lib/public-profiles";
import { useProfile } from "@/lib/profile-store";
import { categoryMeta } from "@/lib/services-store";
import { loadPortfolio, savePortfolio, type PortfolioItem } from "@/lib/portfolio-store";
import { ImageOff, Sparkle } from "lucide-react";

// Fotos de exemplo (Unsplash) — usadas no botão "Carregar exemplos" do estado vazio.
const DEMO_PORTFOLIO: PortfolioItem[] = [
  { id: "demo-1", src: "https://images.unsplash.com/photo-1522337360788-8b13dee7a37e?w=320&q=70&fm=webp&fit=crop", title: "Cabelo liso premium", category: "Cabelo", description: "Escova progressiva com finalização de brilho intenso." },
  { id: "demo-2", src: "https://images.unsplash.com/photo-1503951914875-452162b0f3f1?w=320&q=70&fm=webp&fit=crop", title: "Coloração balayage", category: "Cabelo", description: "Mechas naturais com efeito iluminado." },
  { id: "demo-3", src: "https://images.unsplash.com/photo-1560869713-7d0a29430803?w=320&q=70&fm=webp&fit=crop", title: "Cílios fio a fio", category: "Cílios", description: "Volume russo com curvatura D." },
  { id: "demo-4", src: "https://images.unsplash.com/photo-1487412947147-5cebf100ffc2?w=320&q=70&fm=webp&fit=crop", title: "Maquiagem social", category: "Maquiagem", description: "Look glow para festas e eventos." },
  { id: "demo-5", src: "https://images.unsplash.com/photo-1604654894610-df63bc536371?w=320&q=70&fm=webp&fit=crop", title: "Unhas decoradas", category: "Unhas", description: "Nail art francesinha minimalista." },
  { id: "demo-6", src: "https://images.unsplash.com/photo-1595959183082-7b570b7e08e2?w=320&q=70&fm=webp&fit=crop", title: "Design de sobrancelhas", category: "Sobrancelhas", description: "Mapeamento facial com henna." },
];
import salonBanner from "@/assets/salon-banner.png.asset.json";
...
export const Route = createFileRoute("/agendar/$slug")({
  ...
  head: ({ params, loaderData }) => {
    ...
    const title = p ? `${p.name} — Agendamento online` : "Agendar";
    const desc = p ? `${p.tagline}. Agende online em segundos.` : "Página de agendamento.";
    return {
      meta: [
        { title },
      ...
      links: [
        ...
        // Preconnect para acelerar o portfolio (Unsplash CDN)
        { rel: "preconnect", href: "https://images.unsplash.com", crossOrigin: "" } as const,
        { rel: "dns-prefetch", href: "https://images.unsplash.com" } as const,
        // Banner (LCP) com prioridade alta
        { rel: "preload", as: "image", href: salonBanner.url, fetchpriority: "high" } as const,
        // Avatar da profissional — acima da dobra, alta prioridade
        ...(p?.avatar?.url
          ? [{ rel: "preload", as: "image", href: p.avatar.url, fetchpriority: "high" } as const]
          : []),
      ],
    };
  },
...
function PublicBookingPage() {
  const { profile: initial } = Route.useLoaderData() as { profile: NonNullable<ReturnType<typeof getProfileBySlug>> };
  const profile = useProfile(initial.slug) ?? initial;
  ...
  // Hidrata o portfólio do studio a partir do localStorage (somente cliente).
  // Se não houver fotos cadastradas, a seção de portfólio não é exibida.
  useEffect(() => {
    try {
  ...
  // Simula estado de carregamento dos serviços; em integração real, usar status da query.
  useEffect(() => {
    const t = setTimeout(() => setServicesReady(true), 350);
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="relative overflow-hidden text-white">
        <div className="relative h-[340px] w-full sm:h-[420px]">
          ...
          {/* Badge premium opcional no canto */}
          <div className="relative z-10 mx-auto mt-6 flex w-full max-w-md justify-center px-5">
            <span className="inline-flex items-center gap-1.5 rounded-full border border-white/30 bg-white/10 px-3 py-1 text-[11px] font-medium uppercase tracking-[0.2em] backdrop-blur-md">
              <Sparkles className="h-3 w-3" fill="currentColor" /> Studio Premium
            </span>
          </div>
        </div>
      </header>
              ...
              <div className="relative h-full w-full overflow-hidden rounded-full bg-background p-[3px]">
                {profile.avatar ? (
                  <div className="relative h-full w-full overflow-hidden rounded-full">
                    {/* Placeholder com iniciais enquanto a foto carrega */}
                    <div
                      aria-hidden={avatarLoaded}
                      className={`absolute inset-0 flex items-center justify-center rounded-full bg-card font-display text-5xl font-bold text-gradient transition-opacity duration-500 ${avatarLoaded ? "opacity-0" : "animate-pulse opacity-100"}`}
                    >
                      {profile.initials}
                    </div>
                    <img
                      ...
                      {...({ fetchpriority: "high" } as Record<string, string>)}
                      className={`h-full w-full rounded-full object-cover transition-opacity duration-500 ${avatarLoaded ? "opacity-100" : "opacity-0"}`}
                    />
                  </div>
                ) : (
              ...
              <div className="flex items-center gap-2">
                ...
                {portfolio.length < 4 && (
                  <button
                    type="button"
                    onClick={() => {
                      const merged = [...portfolio, ...DEMO_PORTFOLIO].slice(0, 10);
                      savePortfolio(merged);
                      setPortfolio(merged);
                      toast.success("Exemplos adicionados ao portfólio");
                    }}
                    className="inline-flex items-center gap-1 rounded-full border border-border/60 bg-background px-2 py-0.5 text-[10px] font-medium text-muted-foreground transition hover:text-foreground"
                  >
          ...
          {portfolio.length === 0 ? (
            <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-border bg-secondary/40 px-5 py-10 text-center">
              ...
              <Button
                ...
                onClick={() => {
                  savePortfolio(DEMO_PORTFOLIO);
                  setPortfolio(DEMO_PORTFOLIO);
                  toast.success("Fotos de exemplo carregadas");
                }}
              >
          ...
          {!servicesReady ? (
            <div className="grid grid-cols-2 gap-3">
              {Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="flex h-40 flex-col justify-between rounded-lg border border-border/60 bg-card p-3.5"
                >
                  ...
                  <Skeleton className="h-7 w-7 self-end rounded-full" />
                </div>
              ))}
            </div>
          ) : (
        ...
        <Section
          ...
          right={
            <div className="flex items-center gap-1 text-xs">
              <Star className="h-3.5 w-3.5 text-primary" fill="currentColor" />
              <span className="font-semibold">{profile.rating.toFixed(1)}</span>
              <span className="text-muted-foreground">({profile.reviewsCount})</span>
            </div>
          }
        >
...
function BookingSheet({
  ...
  function handlePaymentConfirmed() {
    ...
    toast.success("Pagamento confirmado!", {
      description: "Agendamento garantido. Confirmação enviada por WhatsApp e e-mail.",
      duration: 6000,
    });
    // Simula envio de confirmação (WhatsApp + e-mail)
    setTimeout(() => {
      toast("Confirmação enviada", {
        description: `${phone ? `WhatsApp ${phone}` : "WhatsApp"}${email ? ` · ${email}` : ""}`,
        icon: "✅",
        duration: 5000,
      });
    }, 1200);
        ...
        {TIMES.map((t) => {
          ...
          return (
            <button
              ...
              disabled={!date}
              className={cn(
                "rounded-2xl border py-3 text-sm font-semibold transition active:scale-95 disabled:opacity-40",
                selected
                  ? "border-primary gradient-primary text-white shadow-glow"
                  : "border-border/60 bg-card hover:border-primary/40",
              )}
