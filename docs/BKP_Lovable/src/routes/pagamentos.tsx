...
import { clients as seedClients, appointments as seedAppointments, services as seedServices, type Client } from "@/lib/mock-data";
...
function PagamentosPage() {
  ...
  const filteredClients = useMemo(() => {
    ...
    if (!q) return seedClients.slice(0, 6);
    return seedClients.filter((c) => c.name.toLowerCase().includes(q) || c.phone.includes(q)).slice(0, 8);
  }, [clientQuery]);

  const selectedClient = useMemo<Client | null>(
    () => seedClients.find((c) => c.id === selectedClientId) ?? null,
    [selectedClientId],
  );
  ...
  const openItems = useMemo(() => {
    ...
    return seedAppointments
      ...
      .map((a) => {
        const svc = seedServices.find((s) => s.id === a.serviceId);
        return {
          id: a.id,
                        ...
                        {filteredClients.map((c) => (
                          <button
                            ...
                            onClick={() => {
                              ...
                              setClientQuery("");
                              const pending = seedAppointments
